...
export class FortyTwoStrategy extends PassportStrategy(Strategy) {
    ...
    async validate(accessToken: string, refreshToken: string, profile: any): Promise<SessionUser> {
        this.logger.verbose(`### Validating user ${profile.id} with 42 strategy`)
        const {id, username, displayName, emails, profileUrl, otpEnabled, otpSecret} = profile;
