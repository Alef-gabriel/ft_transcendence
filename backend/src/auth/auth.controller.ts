...
import {FortyTwoAuthGuard, SessionUser, Public, OTP, TwoFactorAuthentication} from "./index";
...
export class AuthController {
    ...
    @Get('logout')
    async logout(@Req() {user}: { user: SessionUser }, @Res() res: any, @Session() session: any): Promise<void> {
        if (session) {
            session.destroy();
        }
    ...
    async register2FA(@Req() {user}: { user: SessionUser }, @Res() res: any) {
        const otpAuthUrl: string = await this.authService.generate2FASecret(user.id, user.email);
        return this.authService.pipeQrCodeStream(res, otpAuthUrl);
    }

    @Post('2fa/turn-on')
    async turnOn2FA(@Req() {user}: { user: SessionUser }, @Res() res: any, @Body() otp: OTP) {
    ...
    async validate2FA(@Req() req: Request, @Res() res: any, @Body() otp: OTP) {
        ...
        req.logIn({...req.user, otpValidated: true}, function (err: any) {
            ...
            return res.status(200).json({msg: 'Two-factor authentication validated'});
        });
    }
